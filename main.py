...
class CurrencyConverterUI(tk.Tk):
    def __init__(self, converter):
        ...
        # Label
        self.intro_label = Label(self, text = 'Welcome to Real Time Currency Converter', bg = 'grey', fg = 'white',
                                 relief = tk.RAISED, borderwidth = 3)
        self.intro_label.config(font = ('Courier', 15, 'bold'))

        self.date_label = Label(self, text = f"1 Polish zł equals = "
                                             f"{self.currency_converter.convert('PLN', 'USD', 1)} USD \n "
                                             f" Date : {self.currency_converter.data['date']}", relief = tk.SUNKEN,
                                borderwidth = 5)
